...
function scoreTrackAgainstQuery(track, rawQuery) {
...
function prioritizeTracksForPlayback(tracks, rawQuery = '') {
  ...
  return [...tracks].sort((a, b) => {
    const aScore = scoreTrackAgainstQuery(a, rawQuery) + (sourceScore[a?.source] ?? 1);
    const bScore = scoreTrackAgainstQuery(b, rawQuery) + (sourceScore[b?.source] ?? 1);
...
async function searchWithFallbackEngines(query, platform, requestedBy) {
  ...
  for (const searchEngine of resolved.searchEngines) {
    ...
    try {
      ...
      if (result?.hasTracks?.()) {
        result.setTracks(prioritizeTracksForPlayback(result.tracks, resolved.rawQuery || resolved.query));
        return {
          result,
...
async function runSearch(query, platform, requestedBy) {
  ...
  const { result } = await searchWithFallbackEngines(prepared.query, prepared.selectedPlatform, requestedBy);
  return result;
}
...
async function queueAndPlay(voiceChannel, query, textChannel, requestedBy, platform = 'auto') {
  ...
  try {
    ...
    try {
      const bridgeSearch = await searchWithFallbackEngines(prepared.query, 'auto', requestedBy);
    ...
    try {
      ...
      if (soundcloudQuery) {
        const sc = await searchWithFallbackEngines(soundcloudQuery, 'soundcloud', requestedBy);
