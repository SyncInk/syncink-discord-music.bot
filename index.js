...
console.warn = (...args) => {
  ...
  if (first.includes('[YOUTUBEJS][Text]: Unable to find matching run for command run. Skipping...')) return;
...
async function queueAndPlay(voiceChannel, query, textChannel, requestedBy, platform = 'auto') {
  ...
  try {
    ...
    try {
      ...
      bridgeQuery = buildBridgeSearchQuery(fallbackTrack) || rawQuery;
    ...
    try {
      const soundcloudQuery = bridgeQuery || rawQuery;
      if (soundcloudQuery) {
