...
function resolveSearchOptions(query, platform) {
  ...
  const looksLikeUrl = isLikelyUrl(rawQuery);

  let searchEngine = config.searchEngine;

  if (platform === 'auto' && !looksLikeUrl) {
    searchEngine = QueryType.YOUTUBE_SEARCH;
  }

  return {
    decoratedQuery: config.decorateQuery(rawQuery),
    searchEngine,
    label: config.label,
  };
}
...
async function queueAndPlay(voiceChannel, query, textChannel, requestedBy, platform = 'auto') {
  ...
  const baseOptions = {
    ...
    afterSearch: async (searchResult) => {
      if (!searchResult?.hasTracks?.()) return searchResult;
      searchResult.setTracks(prioritizeTracksForPlayback(searchResult.tracks));
      return searchResult;
    },
    nodeOptions: {
      metadata: {
  ...
  try {
    result = await player.play(voiceChannel, resolved.decoratedQuery, baseOptions);
  } catch (primaryError) {
    // Fallback to auto query resolution when platform-specific search fails.
    if (platform !== 'auto') {
